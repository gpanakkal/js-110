...
// const ROYAL_CARD_POINTS = 10;
const DEFAULT_DECK = constants.SUITS.map(suit => constants.CARD_NAMES.map(card => ({name: card, suit}))).flat();
...
function newShuffledDeck() {
  ...
  for (let i = 0; i < constants.SHUFFLE_ITERATIONS; i++) {
...
function twentyOne() {
  while (true) {
    ...
    const playerBusted = score(gameState.playerHand) >= constants.MINIMUM_BUST_SCORE;
...
function resolveGame(gameState) {
  ...
  const playerScore = score(gameState.playerHand);
  const dealerScore = score(gameState.dealerHand);
...
function playerTurn(gameState) {
  while (score(gameState.playerHand) < constants.MINIMUM_BUST_SCORE) {
    const userInputValidator = (userInput) => ['hit', 'stay'].includes(userInput.toLowerCase());
    const playerChoice = getValidInput(constants.USER_TURN_PROMPT, question, constants.INVALID_TURN_MESSAGE, userInputValidator);
...
function dealerTurn(gameState) {
  while (score(gameState.dealerHand) < constants.DEALER_STAY_SCORE) gameState.dealerHand.push(...gameState.deck.splice(0, 1));
}
/** status: testing
 * Convert a player's hand from an array of card objects to a string of the form "x of <suit>, y of <suit>, and z <of suit>"
 */
function formatHandAsString(hand) {
  switch (hand.length) {
    case 0: return 'no cards';
    case 1: return hand.map(formattedCard)[0];
    case 2: return hand.map(formattedCard).join(" and ");
    default:
...
/** status: done
 * Prompt the user for input and reprompts the user until they provide input that passes the condition in the validation callback
 * @param {string} prompt 
